describe('Visits exams list', () => {
  ...
  it('And sees the info ordered by name, separated by person, then for exam and the list os exams for each token', () => {
    cy.request('POST', `/populate_test_db`);
    cy.visit('/exams');

    cy.window().then((win) => {
      const originalQueriesNew = win.Queries.new;

      cy.stub(win.Queries, 'new').callsFake((config_file, options) => {
        options.scope = 'test';
        return originalQueriesNew.call(win.Queries, config_file, options);
      });
    });
